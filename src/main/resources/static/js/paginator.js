...
function paginator(config) {
    ...
    if (typeof config.box_mode == "function") {
        ...
        if (config.box_mode == "list") {
            ...
            make_button = function (symbol, index, config, disabled, active) {
                ...
                button.classList.add("btn", "btn-default"); //added bootstrap class(es)
    ...
    if (!(typeof config.page_options == "boolean" && !config.page_options)) {
        ...
        select.classList.add("form-control"); //added bootstrap class(es)

        //adding styles
        select.style.display = "inline-block";
        select.style.width = "auto";
        select.style.margin = "1px 10px 1px 10px";
        ...
        page_box.appendChild(select); //change from "box" to "page_box"
    }
